#!/usr/bin/env python3
"""
Minimal BitFit fine-tuning script with NVML energy logging.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Minimal BitFit fine-tuning")
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Substring pattern(s) for parameters to keep trainable (default: bias only)",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    patterns = args.trainable_pattern or ["bias"]
    ...
    if trainable_params == 0:
        raise ValueError(
            f"No parameters matched {patterns}. Try a broader pattern such as 'bias' or 'ln_f.bias'."
        )
    print(
        f"Trainable parameters: {trainable_params:,} / {total_params:,} "
        f"({trainable_params / total_params * 100:.4f}% match {patterns})"
    )
